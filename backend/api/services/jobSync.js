...
const { extractJobSkills } = require("./jobMatching");

const JOBS_URL =
  "https://raw.githubusercontent.com/SimplifyJobs/Summer2026-Internships/dev/.github/scripts/listings.json";
const JOB_SYNC_BATCH_SIZE = 500;
const JOB_SYNC_FRESHNESS_MINUTES = 15;
const JOB_SYNC_TIMEOUT_MS = 15000;

let syncInFlight = null;

const buildSyncOperations = (jobs, timestamp) =>
  jobs
    .map((job) => {
...
const syncJobsFromUpstream = async ({ force = false } = {}) => {
  if (syncInFlight) {
    return syncInFlight;
  }
  ...
  syncInFlight = (async () => {
    ...
    const now = new Date();
    const operations = buildSyncOperations(data, now);
